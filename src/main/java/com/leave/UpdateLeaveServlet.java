...
public class UpdateLeaveServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		boolean isTrue;
		isTrue = LeaveController.updateData(Integer.parseInt(attendanceId), attendanceDate, status, leaveType,
				leaveReason);
		...
		if (isTrue == true) {
			...
			response.getWriter().println("<script> alert('" + alertMessage
					+ "'); window.location.href='/Employee_Management_System/hrmanager/leaverecordsdisplay.jsp'</script>");
		} else {
